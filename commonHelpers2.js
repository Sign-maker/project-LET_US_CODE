import{S as _,F as k,a as p}from"./assets/subscription-df046af3.js";import"./assets/vendor-bb007519.js";const n="/project-LET_US_CODE/assets/icons-b681127c.svg";function A(t){return t.map(({category:e,img:o,is10PercentOff:c,name:s,popularity:r,price:a,size:l,_id:g})=>`<li class="products-card-item" data-id="${g}">
      ...
      </li>`}).join("")}const G="filter-storage",F="category-storage",N="product-storage",V="popularity-storage",H="discount-storage",U="shop-storage",z={keyword:null,category:null,page:1,limit:9},W=document.querySelector(".product-card-list"),Y=document.querySelector(".popular-list"),Q=document.querySelector(".discount-list"),J=document.querySelector(".content-wrapper"),K=document.querySelector(".js-header-navSpan"),v=new k,E=new p(G),X=new p(F),b=new p(N),w=new p(V),S=new p(H),$=new _(U);J.addEventListener("click",at);const Z=E.getItem??z;tt(Z);function tt(t){et(),ot(t),st(),ct()}C($.getAllProducts());async function et(){try{const t=await v.getProductCategories();X.setValue(t)}catch(t){console.log(t)}finally{}}async function ot(t){try{const e=await v.getProducts(t);b.setValue(e),E.setValue(t),I(W,b.getValue().results)}catch(e){console.log(e)}finally{}}async function st(){try{const t=await v.getPopularProducts();w.setValue(t),j(Y,w.getValue())}catch(t){console.log(t)}finally{}}async function ct(){try{const t=await v.getDiscountedProducts();S.setValue(t),M(Q,S.getValue())}catch(t){console.log(t)}finally{}}async function rt(){const t=document.getElementsByClassName("js-card-item");console.log(t)}rt();function at(t){if(!t.target.closest(".js-add-btn"))return;const o=t.target.closest("li").dataset.id;if(nt(o))return;t.target.closest("button").classList.add("is-added"),t.target.closest(".popular-btn")&&(t.target.closest(".popular-btn").style.backgroundColor="#6d8434");const s=b.getValue().results,r=w.getValue(),a=S.getValue(),l=y(s,o),g=y(r,o),m=y(a,o);switch("object"){case typeof l:shopStorage.setProduct(l);break;case typeof g:shopStorage.setProduct(g);break;case typeof m:shopStorage.setProduct(m);break;default:alert("Нет таких значений")}C($.getAllProducts()),console.log(o)}function nt(t){return shopStorage.getAllProducts().some(o=>o._id===t)}function y(t,e){return t.find(c=>c._id===e)}function C(t){let e=t.length;K.textContent=e}const u=new k;async function lt(){try{const t=await u.getProductCategories();console.log("productCategories",t);const e={keyword:"apple",category:"Fresh_Produce",byABC:!0,byPrice:!0,byPopularity:!0,page:1,limit:6},o=await u.getProducts(e);console.log("products",o);const c="640c2dd963a319ea671e3676",s=await u.getProductDetails(c);console.log("productsDetails",s);const r=await u.getPopularProducts();console.log("popularProducts",r);const a=await u.getDiscountedProducts();console.log("discountedProducts",a);const l={email:"test@gmail.com",products:[{productId:"640c2dd963a319ea671e383b",amount:2}]},g=await u.placeOrder(l);console.log("placeOrder",g);const m={email:"12345@cdfbgdfgbhdfgh.com"},q=await u.subscribeToNewsletter(m);console.log("subscribeToNewsletter",q)}catch(t){console.log("помилка",t.response.data.message)}finally{}}lt();const dt=document.querySelector(".product-card-list");dt.addEventListener("click",it);function it(t){t.preventDefault();const e=t.target;if(e.nodeName==="IMG"||e.nodeName==="H4"){const o=e.closest(".products-card-item").dataset.id;pt(o)}else return}const f=document.querySelector(".modal-prod-wrapper"),ut="shop-storage";new p(ut);async function pt(t){console.log(t);const o=await new k().getProductDetails(t);gt(o)}async function gt(t){try{f.classList.add("modal-active"),document.body.classList.add("stop-scroll"),f.innerHTML=`
  ...
  `,document.querySelector(".modal-order-succ-btn").addEventListener("click",L),window.addEventListener("click",T),window.addEventListener("keydown",B)}function L(){h.classList.remove("modal-show-order"),document.body.classList.remove("stop-scroll"),window.removeEventListener("click",T),window.removeEventListener("keydown",B)}function T(t){t.target===h&&L()}function B(t){t.keyCode===27&&L()}
