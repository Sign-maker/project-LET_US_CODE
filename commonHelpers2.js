import"./assets/styles-4c66c0da.js";import{a as r}from"./assets/vendor-a61d8330.js";r.defaults.baseURL="https://food-boutique.b.goit.study/api";class g{constructor(){}async getProductCategories(){const{data:e}=await r("/products/categories");return e}async getProducts(e){const{data:s}=await r("/products",{params:{...e}});return s}async getProductDetails(e){const{data:s}=await r(`/products/${e}`);return s}async getPopularProducts(){const{data:e}=await r("/products/popular");return e}async getDiscountedProducts(){const{data:e}=await r("/products/discount");return e}async subscribeToNewsletter(e){return await r.post("/subscription",e,{headers:{"Content-Type":"application/json"}})}async placeOrder(e){const{data:s}=await r.post("/orders",e,{headers:{"Content-Type":"application/json"}});return s}}const d=new g;async function k(){try{const t=await d.getProductCategories();console.log("productCategories",t);const e={keyword:"apple",category:"Fresh_Produce",byABC:!0,byPrice:!0,byPopularity:!0,page:1,limit:6},s=await d.getProducts(e);console.log("products",s);const o="640c2dd963a319ea671e3676",c=await d.getProductDetails(o);console.log("productsDetails",c);const a=await d.getPopularProducts();console.log("popularProducts",a);const i=await d.getDiscountedProducts();console.log("discountedProducts",i);const n={email:"test@gmail.com",products:[{productId:"640c2dd963a319ea671e383b",amount:2}]},p=await d.placeOrder(n);console.log("placeOrder",p);const P={email:"test@gmail.com"},w=await d.subscribeToNewsletter(P);console.log("subscribeToNewsletter",w)}catch(t){console.log("помилка",t.response.data.message)}finally{}}k();const m=new g,y=document.querySelector(".popular-list"),f=document.querySelector(".discount-list");async function L(){try{const t=await m.getPopularProducts();h("popularProducts",t),y.innerHTML=O(t)}catch{}}L();async function A(){try{const t=await m.getDiscountedProducts();h("discountProducts",t),f.innerHTML=S(t)}catch{}}A();y.addEventListener("click",C);function C(t){!t.target.closest("li")||t.target.closest("button")||console.dir("THIS IS FOR MODAL CLICK")}f.addEventListener("click",x);function x(t){!t.target.closest("li")||t.target.closest("button")||console.dir("THIS IS FOR MODAL CLICK")}function h(t,e){localStorage.setItem(`${t}`,JSON.stringify(e))}function O(t){return t.map(e=>{const{_id:s,name:o,img:c,category:a,size:i,popularity:n}=e;return`<li class="popular-item" data-id="${s}">
      ...
      </li>`}).join("")}function S(t){return t.slice(0,2).map(s=>{const{_id:o,name:c,img:a,price:i}=s;return`<li class="discount-item" data-id="${o}">
      ...
      </li>`}).join("")}const l=document.querySelector(".modal-prod-wrapper");function $(t){console.log(t),l.classList.add("modal-active"),document.body.classList.add("stop-scroll"),l.innerHTML=`
